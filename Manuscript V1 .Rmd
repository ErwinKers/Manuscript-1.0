...
Plotting the CTD data using 
## CTD data
```{r CTD data, echo=FALSE, fig.align=, fig.height=6, fig.keep='all', fig.width=10, message=FALSE, warning=FALSE, paged.print=FALSE, results='hide'}

#Tidying the data frames
setwd("C:/Users/Erwin/OneDrive/Manuscript Erwin Kers/R script + data/Data")
...
## combine plots ----
plot_grid(p1, p2,p4,condbar, labels = c('A', 'B', "C","D"))

```



## Environmental observations (attached to the CTD plots)
```{r echo=FALSE,results='hide',fig.keep='all', fig.height = 4, fig.width = 8, fig.align = "center"}
#conditions
library(readxl)
library(dplyr)
library(ggthemes)
cond <- read_excel("C:/Users/Erwin/OneDrive/Manuscript Erwin Kers/R script + data/Data/Environmental observations/Environmental observations.xlsx")
...
```{r echo=FALSE,results='hide',fig.keep='all', fig.height = 4, fig.width = 8, fig.align = "center"}
water <- read.csv2("C:/Users/Erwin/OneDrive/UIT/Thesis/Light sensor data/Light sensor data - LakeIce, Takvatn/Water light sensor Winter/WATER sensor.csv")
...
lims <- as.POSIXct(strptime(c("2020-11-20 08:09:00", "2021-07-15 17:31:00"), 
                   format = "%Y-%m-%d %H:%M"))
...
light <- ggplot(sub.water,aes(Datefix,divided.10000))+
  ...
  labs(title="Light sensor", y = "Quantum [umol/(m^2s)]",x = "Month") +
xlim(c(as.POSIXct('2020-11-20 08:09', format = "%Y-%m-%d %H:%M"),
     as.POSIXct('2021-07-15 17:31', format = "%Y-%m-%d %H:%M")))
...
library(tidyverse)
library(ggbreak)
nutrient <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/Nutrient data/nutrient data.xlsx")
...
library(tidyverse)
library(cowplot)
Chla <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/Chl-a/Chl-a for R.xlsx")
...
phy <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/Phytoplankton community/Phytoplankton community.xlsx")
library(ggplot2)
library(tidyverse)
library(forcats)
library(ggthemes)
...
### Biomass zooplankton
Biom <-read.csv2("C:/Users/Erwin/OneDrive/UIT/Thesis/Samples/Zooplankton biomas/Zooplankton Biomass.csv")
...
```{r echo=FALSE,results='hide',fig.keep='all', fig.height = 4, fig.width = 8, fig.align = "center"}
Zoopcomp <- read.csv2("C:/Users/Erwin/OneDrive/UIT/Thesis/Samples/Zooplankton ID complete.csv")
...
E <- q %>%
  ...
  scale_x_date(breaks="month", labels = scales::label_date("%b"),limits = as.Date(c('2020-10-20','2021-06-15')))+
    facet_wrap(~Depth2)+ 
  labs(title="Eudiaptomus", y = "N per vertical meter", x= "")+
...
C <- q %>%
  ...
  scale_x_date(breaks="month", labels = scales::label_date("%b"),limits = as.Date(c('2020-10-20','2021-06-15')))+
    facet_wrap(~Depth2)+ 
  labs(title="Cyclops", y = "N per vertical meter", x= "")+
...
## Total lipids over time and total FA overtime
```{r echo=FALSE,results='hide',fig.keep='all', fig.height = 4, fig.width = 8, fig.align = "center"}
...
library(lubridate)
library(scales)
FA <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/FA data/FA data.xlsx", skip=1)
...
```
There is a large difference between the previous and next plot, in the Total lipids plot it seems Cyclops has an higher lipid content. Whn looking at the FA classes if you'd add them up Eudiaptomus has higher content of FAME. This makes me wonder is it normal to have such a difference between to two species in glycerol content? Is this purely due to the difference in storage and membrane lipds?
...
#make a facetwrap
FALIN <- FA.mute %>% 
  tidyr::gather("Legend", "value", 59:63) %>% 
...
FABAR2 <- FA.mute %>% 
  ...
  geom_bar(position="stack", stat="identity")+
  #scale_y_continuous(trans='log10') +
  labs(title="Zooplankton", y = "FAME μg/mg dryweight")+
 facet_wrap(~Species)
FABAR2
```

## FA per L of POM
I have a meeting next week with Matthias @ Wassercluster lunz to discuss if this is correct.
```{r echo=FALSE,results='hide',fig.keep='all',fig.height = 4, fig.width = 8, fig.align = "center"}
library(readxl)
FA <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/FA data/FA data.xlsx", skip=1)
#changing data format
...
library(readxl)
library(easyCODA)
FA.dat <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/FA data/FA data.xlsx", skip=1)
...
FA_data <- read_excel("C:/Users/Erwin/OneDrive/UIT/Data/FA data/FA data.xlsx", skip=1)

...
p1 <- ctd_mba %>%
  ...
  coord_cartesian(expand = 0)+
    ...
    # adding the ice cover bar
  annotate (geom = "rect" ,xmin = as.Date('2020-12-01 08:09', format = "%Y-%m-%d %H:%M"), xmax = as.Date('2021-6-01 08:09', format = "%Y-%m-%d %H:%M"), ymin = -0.1, ymax = -3, fill = "blue", alpha = 0.3) + 
  annotate("text", x=as.Date('2021-3-01 08:09', format = "%Y-%m-%d %H:%M"), y=-1 , label= "Ice cover")
...
## combine plots ---- (Adds light plot, modified to fit in with other plots)
plot_grid(p1, p2,p4,light, labels = c('A', 'B', "C", "D"))
...
light <- ggplot(sub.water,aes(Datefix,divided.10000))+
  ...
  labs( y = "Quantum [umol/(m^2s)]",x = "date") +
...
phybar2 <- phy %>% 
  ...
  ggplot(., aes(x = Date, y = values, fill = Class )) +
...
## !!! combined plot !!!!!
```{r echo=FALSE,results='hide',fig.keep='all', fig.height = 8, fig.width = 12, fig.align = "center"}

plot_grid( chlbar,  plotrot2, biombar, phybar2,  labels = c('A', 'B', "C", "D"), align = "hv")
...
# assign a letter to the plot for use in plot grid
library(gridGraphics)
library(grid)
grid.echo()
c <- grid.grab()
